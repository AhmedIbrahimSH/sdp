...
class FoodNeed extends NeedTemplateMethod
{
    ...
    {
        // Check if the beneficiary ID already exists in the FoodNeedHistory table to update accpeted column
        $query = "SELECT COUNT(*) FROM FoodNeedHistory WHERE BeneficiaryID = ? AND Accepted = 0";
        ...
        if ($count > 0) {
            if ($accepted) {
                ...
                $query = "UPDATE FoodNeedHistory SET Accepted = 1 WHERE BeneficiaryID = ? AND Accepted = 0";
                $stmt = $this->dbConnection->prepare($query);
                $stmt->bindParam(1, $beneficiaryID, PDO::PARAM_INT);
                if ($stmt->execute()) {
