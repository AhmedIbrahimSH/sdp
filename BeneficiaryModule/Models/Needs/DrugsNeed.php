...
class DrugsNeed extends NeedTemplateMethod
{
    ...
    {
        return $beneficiary->hasChronicDisease() && $beneficiary->getIncome() < self::getIncomeThreshold();
    }
    ...
    protected function allocateResources($table, $beneficiaryID)
    {
        ...
        $query = "UPDATE charity_storage SET Amount = Amount - ?, Spendings = Spendings + ?, AffectedPeople = AffectedPeople + ? WHERE type = 'Drugs'";
        $stmt = $this->dbConnection->prepare($query);
