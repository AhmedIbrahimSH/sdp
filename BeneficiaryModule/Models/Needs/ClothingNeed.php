...
class ClothingNeed extends NeedTemplateMethod
{
    ...
    protected function register($beneficiaryID, $amount, $accepted)
    {
        ...
        // Check if the beneficiary ID already exists in the ClothingNeedHistory table to update accpeted column
        $query = "SELECT COUNT(*) FROM ClothingNeedHistory WHERE BeneficiaryID = ? AND Accepted = 0";
        ...
        if ($count > 0) {
            if ($accepted) {
                ...
                $query = "UPDATE ClothingNeedHistory SET Accepted = 1 WHERE BeneficiaryID = ? AND Accepted = 0";
                $stmt = $this->dbConnection->prepare($query);
                $stmt->bindParam(1, $beneficiaryID, PDO::PARAM_INT);
                if ($stmt->execute()) {
            ...
            $query = "INSERT INTO ClothingNeedHistory (BeneficiaryID, Amount, Allocated, Accepted)
         VALUES (?, ?, ?, ?)";
            $stmt = $this->dbConnection->prepare($query);
    ...
    protected function allocateResources($table, $beneficiaryID)
    {
        ...
        $query = "UPDATE charity_storage SET Amount = Amount - ?, Spendings = Spendings + ?, AffectedPeople = AffectedPeople + ? WHERE type = 'Clothing'";
        $stmt = $this->dbConnection->prepare($query);
