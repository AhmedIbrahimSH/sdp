...
class ShelterNeed extends NeedTemplateMethod
{
    ...
    {
        // Check if the beneficiary ID already exists in the ShelterNeedHistory table to update accpeted column
        $query = "SELECT COUNT(*) FROM ShelterNeedHistory WHERE BeneficiaryID = ? AND Accepted = 0";
        ...
        if ($count > 0) {
            if ($accepted) {
                ...
                $query = "UPDATE ShelterNeedHistory SET Accepted = 1 WHERE BeneficiaryID = ? AND Accepted = 0";
                $stmt = $this->dbConnection->prepare($query);
                $stmt->bindParam(1, $beneficiaryID, PDO::PARAM_INT);
                if ($stmt->execute()) {
            ...
            $query = "INSERT INTO ShelterNeedHistory (BeneficiaryID, Amount, Allocated, Accepted)
         VALUES (?, ?, ?, ?)";
            $stmt = $this->dbConnection->prepare($query);
    ...
    protected function allocateResources($table, $beneficiaryID)
    {
        ...
        $query = "UPDATE charity_storage SET Amount = Amount - ?, Spendings = Spendings + ?, AffectedPeople = AffectedPeople + ? WHERE type = 'Shelter'";
        $stmt = $this->dbConnection->prepare($query);
